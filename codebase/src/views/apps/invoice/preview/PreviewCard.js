...
const PreviewCard = ({ data, userData }) => {
  ...
  const ElementsPriceTotaler = (products) => {
    return products
      .reduce(
        (a, b) => +a + +Number(totalHandler(b.productPrice.split("/")[1])),
        0
      )
      .toFixed(2);
  };
      ...
      <CardBody className="invoice-padding pb-0">
        ...
        <div className="d-flex justify-content-between flex-md-row flex-column invoice-spacing mt-0">
          <div>
            ...
            <p className="card-text mb-25">Luis Felipe Villaran 1010 - 102</p>
            ...
            <br />
            <br />
            <p className="card-text mb-25">{data.name}</p>
            <p className="card-text mb-25">
              {data.district} <br /> {data.direction}
            </p>
                ...
                <tr key={`newProducts-${index}`}>
                  ...
                  <td className="py-1">
                    <span className="font-weight-bold">
                      ...
                      {item.product.brand
                        ? `${numberFormat(
                            parseInt(item.product.brand.price)
                          )}.00`
                  ...
                  <td className="py-1">
                    <span className="font-weight-bold">
                      ...
                      {item.product.brand
                        ? `${numberFormat(
                            parseInt(
                              Number(item.product.brand.price) *
                                Number(item.qty)
                            )
                          )}.00`
                        : `${numberFormat(
            ...
            <div className="invoice-total-wrapper">
              ...
              <div className="invoice-total-item">
                ...
                <p className="invoice-total-amount">
                  ...
                  {data.elements
                    ? `${numberFormat(
                        parseInt(ElementsPriceTotaler(data.elements))
                      )}.00`
                    : data.products
                    ? `${numberFormat(
                        parseInt(ProductsPriceTotaler(data.products))
                      )}.00`
                    : null}
                </p>
              </div>

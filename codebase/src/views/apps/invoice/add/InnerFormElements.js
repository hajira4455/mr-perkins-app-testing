...
function InnerFormElements({
  ...
  const GetPriceInNumber = (selected, selectedPacking, multiple) => {
    if (selected !== {}) {
      ...
      dispatch(
        SetInvoiceData({
          ...
          product: {
            ...
            total: Number(multiple) * Number(selectedPacking) * Number(price),
          },
        })
      );
      return Number(multiple) * Number(selectedPacking) * Number(price);
    }
  };
  return (
        ...
        <Row className="w-100 pr-lg-0 pr-1 py-2">
          ...
          <Col className="my-lg-0 mt-2" lg="2" sm="12">
            ...
            <CardText className="mb-0">
              S/{" "}
              {numberFormat(
                parseInt(GetPriceInNumber(selected, selectedPacking, multiple))
              )}
              {".00"}
            </CardText>
          </Col>
